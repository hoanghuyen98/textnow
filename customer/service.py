...
print('settings.SELLMMO_KEY: ', settings.SELLMMO_KEY)
...
def fetch_categories(provider: str):
    ...
    return {"success": True, "provider": provider, "count": len(result), "data": result}
...
def buy_mail_sellmmo(employee, product_id: str, amount: int = 1, coupon: str = ""):
    ...
    try:
        ...
        return {"status": "error", "message": f"Lỗi khi gọi API SellMMO: {e}"}
...
def buy_mail_dongvan(employee, account_type: int, quality: int = 0, type: str = "full"):
    ...
    try:
        ...
        return {"status": "error", "message": f"Lỗi khi gọi API DongVan: {e}"}
